...
| --- | --- |
| 应用包名 | `io.legado.read` |
| 当前版本 | `3.9.905`（以 `AppScope/app.json5` 为准） |
...
- 支持可选的本地眼动翻页：优先使用设备可用的 AR Engine 眼部信号，不支持时回退到系统人脸检测，完成纵向注视识别；提供横竖屏独立校准、上下边缘停留时长和防误触冷却。相机画面仅在设备内处理，不上传或保存，滚动模式及菜单、选区、加载等交互期间不会触发翻页。
...
- 书源、书架、章节进度、阅读设置、主题、书架展示配置和 TTS 配置按各自策略保存；书籍置顶状态纳入本地备份和账号云同步；相同本地书籍通过内容指纹跨设备匹配并仅同步阅读进度，不上传本地正文；眼动开关、横竖屏校准和停留时长纳入阅读设置与账号云同步，旧云端设置会保留本机新增字段并安排兼容回填。
...
| `versionName` | `3.9.905` |
| `versionCode` / `buildVersion` | `390905` |
| `minAPIVersion` | `12` |
| `targetAPIVersion` | `26` |
