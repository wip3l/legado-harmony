...
开源轻页是一个面向 HarmonyOS 的本地阅读与书源规则兼容项目，目标是验证用户自行提供、且有合法使用权限的规则在移动端的导入、搜索、发现、详情解析、目录解析和阅读体验。

项目使用 ArkTS 和 ArkUI 开发，当前包名为 `io.legado.read`，支持 phone 和 tablet。

## 合规说明

- 本项目不内置、不托管、不分发任何小说正文、章节内容、封面图片或付费资源。
- 本项目不提供书源订阅服务，也不推荐、维护或背书任何第三方内容站点。
- 用户导入书源前，应自行确认书源来源、目标站点协议、内容授权和所在地法律法规要求。
- 本项目仅按用户配置的规则发起访问和解析，不以破解付费内容、绕过访问控制、规避反爬机制或批量抓取受保护内容为目标。
- 如权利人认为项目中的示例、说明或调试材料存在不当之处，可通过 Issue 反馈，项目将及时处理。

## 功能概览

- 书架：管理已加入书籍，支持打开阅读、删除书籍和本地状态刷新。
- 搜索：基于已启用书源进行并发搜索，解析书名、作者、分类、封面和简介。
- 发现：读取书源中的发现入口，按站点和分类浏览内容。
- 书源管理：支持从 URL、剪贴板或文本文件导入 JSON 书源，支持启用、编辑和调试。
...
- 桌面快捷项：长按应用图标可选择「最近阅读」，直接打开最近一次阅读的书籍。
- 桌面服务卡片：在桌面卡片中显示本地最近阅读书名列表，点击书名可回到对应书籍继续阅读。

## 当前状态

这是一个正在迭代中的阅读器项目，核心目标是提升规则解析、阅读排版和本地书架体验。请仅导入自有、授权或明确允许访问的书源规则。
...
1. 安装应用后进入「我的」。
2. 打开「书源管理」。
3. 导入自有或已获授权的 JSON 书源。
4. 返回首页后使用「搜索」查找书籍。
5. 进入书籍详情，加入书架或直接阅读。
6. 在阅读页点击中间区域呼出菜单，进入阅读设置调整排版和翻页方式。

发现页依赖书源中的 `exploreUrl` / `ruleExplore` 配置。若某个书源没有发现规则、规则依赖站点动态能力，或目标站点不允许自动化访问，该书源可能不会在发现页返回内容。
...
项目根目录保留了 `debug_source.json`，仅用于本地调试规则兼容。使用或替换调试材料前，请确认来源和目标站点授权。
