...
书源字段、请求格式和解析规则的完整说明见 [《书源开发文档》](../book-source-development.md)。
...
| 发现地址 | `龙族::/novels/api/book/search?keyword=%E9%BE%99%E6%97%8F&pi={{page}}&ps=30` |
...
5. 保存书源。
6. 回到「发现」，确认可看到「龙族」等分类及分类下书籍。
7. 回到「搜索」，搜索任意书名，例如「龙族」，选择「酷我小说」结果打开。
8. 进入详情页后可加入书架并开始阅读。
...
4. 点击「从 URL 导入」。
5. 粘贴上面的 URL，确认导入。
6. 提示成功导入 1 个书源后，进入「发现」和「搜索」验证。

如果当前上架审核包对应的代码不在 `main` 分支，请将 URL 中的 `main` 替换为实际分支名或 tag。

## 方式三：从文件导入

1. 将工程文件 `docs/book-sources/kuwo-novel.json` 复制到手机可访问的位置。
2. 打开应用，进入「我的」。
3. 进入「书源管理」。
4. 点击「从文件导入」。
5. 在文件选择器中选择 `kuwo-novel.json`。
6. 提示成功导入 1 个书源后，进入「发现」和「搜索」验证。
...
1. 导入「酷我小说」书源。
2. 进入「发现」，打开「龙族」分类，确认列表有内容。
3. 进入「搜索」，搜索「龙族」。
4. 打开任意搜索结果。
5. 点击「加入书架」。
6. 点击「开始阅读」，确认目录和正文能够正常加载。
